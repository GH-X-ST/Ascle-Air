function [r, Ct_j, Cp_j, dCt_j, dCp_j, lambda_induced_j, lambda_j, dCl_j, AoA, theta_0_u, Cp_induced_j, Cp_profile_j] = BEMT_axial_UpperRotor(theta_tw_u, TR_u, Vc, Ct_u_req, polar)
...
% for i = 1:10
    ...
    for j = 2:length(r)-1
        ...
        dCt_j2(j) = Nb*0.5*rho*U^2*c_u(r(j))*Cl*dr*R/(rho*Ae*Vtip^2);
        
        % incremental power
        dCp_j(j) = dCt_j(j)*lambda_j(j);
...
theta_0_u = theta_0_u*180/pi;

% disp([' ----------- Upper rotor -----------']);
% disp(['Collective pitch angle initial estimate at Vc = ', num2str(Vc), 'm/s is: ', num2str(theta_0_u_initial*180/pi), ' degree.']);
% disp(['Converged value: ', num2str(180*theta_0_u_new/pi), ' degree.']);
% disp(['CT: ', num2str(Ct_j),'; Required CT: ',num2str(Ct_u_req)]);
% disp(['CP/CQ: ', num2str(Cp_j)]);
